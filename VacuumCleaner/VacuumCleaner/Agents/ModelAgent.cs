...
{
    public class ModelAgent : IAgent
    {
        ...
        /// <summary>
        /// Total number of cleaned tiles
        /// </summary>
        private int CleanedDirty = 0;
        /// <summary>
        /// Sum of all times between current iteration and last clean
        /// </summary>
        private long TotalDirtTime = 0;
        /// <summary>
        /// Approximate number of steps when dirt will appear on tile after last clean
        /// </summary>
        private double DirtExpectation;
        ...
        public ActionType Think()
        {
            ...
            if (IsDirty)
            {
                ...
                if (Phase1Running)
                {
                    lastClean = HugeMap[CurrentPos.Row, CurrentPos.Col].LastClean;
                    HugeMap[CurrentPos.Row, CurrentPos.Col].LastClean = CurrentIteration;
                }
                else
                {
                    lastClean = SmallMap[CurrentPos.Row, CurrentPos.Col].LastClean;
                    SmallMap[CurrentPos.Row, CurrentPos.Col].LastClean = CurrentIteration;
                }

                //Recalculating dirt expectation
                TotalDirtTime += (CurrentIteration - lastClean);
                DirtExpectation = (double)TotalDirtTime / CleanedDirty;
